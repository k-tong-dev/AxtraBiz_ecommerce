...
export function ProductBarcodePrintTemplate({data}: ProductBarcodePrintTemplateProps) {
    ...
    const BarcodeLabel = ({product}: {product: any}) => {
        const barcodeValue = product.barcode || product.barcode_number || product.sku || product.code || 'SKU-001'
        ...
        return (
            <div className="shadow-sm rounded-sm p-4 bg-white" style={{width: '100mm', height: '50mm'}}>
                <div className="text-center h-full flex flex-col justify-center">
                    <h3 className="font-bold text-lg mb-2" style={{fontSize: '14pt'}}>{product.name || 'Product'}</h3>
                    <div className="text-sm text-gray-600 mb-2" style={{fontSize: '10pt'}}>{barcodeValue}</div>
