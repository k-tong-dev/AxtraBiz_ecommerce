...
function findOption(options: SelectOption[], value: string): SelectOption | undefined {
  for (const opt of options) {
    ...
    if (opt.children) { const f = findOption(opt.children, value); if (f) return f }
  }
}
...
export function SelectionField({ config, value, onChange, error }: FieldProps) {
  ...
  const pickerProps = { data, searchable: config.searchable !== false, loading, placeholder: config.placeholder || ' ', disabled: config.readonly, style: PICKER_STYLE, onOpen: () => setOpen(true), onClose: () => setOpen(false) }
