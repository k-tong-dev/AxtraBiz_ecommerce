...
import {resShops, resUsers, resGroups, m2mUsersShops, m2mUsersGroups, type ResShop} from '@/lib/drizzle/schema'
...
export async function createShopWithOwner(
    ...
    const [shopAdminGroup] = await db.select({id: resGroups.id})
        .from(resGroups)
        .where(eq(resGroups.name, 'Shop Admin'))
        .limit(1)

    if (shopAdminGroup) {
        await db.insert(m2mUsersGroups)
            .values({userId: existingUser.id, groupId: shopAdminGroup.id})
            .onConflictDoNothing()
    }
