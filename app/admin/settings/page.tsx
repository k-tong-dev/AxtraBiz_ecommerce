...
export default function AdminSettingsPage() {
  ...
  const title = useMemo(() => (loading ? 'Settings (loading...)' : 'Settings'), [loading])
    ...
    <Card className="mx-auto max-w-7xl border-border/50">
      <div className="flex items-center justify-between gap-4 px-6 pt-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Key/value store for admin configuration.</p>
        </div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{mode === 'create' ? 'Add Setting' : 'Edit Setting'}</DialogTitle>
