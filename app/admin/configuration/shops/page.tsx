...
export default function AdminShopsPage() {
  ...
  const { data: shops, loading, refresh } = useResource<Shop[]>('/api/admin/shops')
  const { confirmDelete, deleteModal } = useConfirmDelete({ apiEndpoint: '/api/admin/shops', entityName: 'shop', refresh, useQueryParam: true })

  const openCreate = () => router.push('/admin/configuration/shops/new')
  const openEdit = (row: Shop) => router.push(`/admin/configuration/shops/${row.id}/edit`)
