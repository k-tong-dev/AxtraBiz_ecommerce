        ...
        <div className="w-full max-w-md justify-self-center lg:order-2">
          <div className="space-y-6 rounded-3xl border border-border/60 bg-card/90 p-8 shadow-2xl backdrop-blur">
            ...
            <Button
              ...
              onClick={async () => {
                ...
                if (created.needsVerification) {
                  ...
                  setTimeout(() => {
                    router.push(`/shop/verify-otp?email=${encodeURIComponent(email)}&redirect=${encodeURIComponent(redirect)}`)
                  }, 1500)
                } else {
            ...
            <p className="text-sm text-muted-foreground">
              ...
              <Link href={`/shop/login?redirect=${encodeURIComponent(redirect)}`} className="font-medium text-primary hover:underline">
                Sign in
              </Link>
            </p>
          </div>
