...
export default function AdminCouponsPage() {
  ...
  const { data: coupons, loading, refresh } = useResource<Coupon[]>('/api/dashboard/coupons')
  const { confirmDelete, deleteModal } = useConfirmDelete({ apiEndpoint: '/api/dashboard/coupons', entityName: 'coupon', refresh, useQueryParam: true })

  const openCreate = () => router.push('/dashboard/marketing/coupons/new')
  const openEdit = (row: Coupon) => router.push(`/dashboard/marketing/coupons/${row.id}/edit`)
