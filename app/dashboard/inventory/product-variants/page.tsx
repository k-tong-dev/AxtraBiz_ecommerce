...
export default function AdminProductVariantsPage() {
  ...
  const { data: variants, loading, refresh } = useResource<ProductVariant[]>('/api/dashboard/product-variants')
  const { confirmDelete, deleteModal } = useConfirmDelete({ apiEndpoint: '/api/dashboard/product-variants', entityName: 'product variant', refresh, useQueryParam: false })

  const openCreate = () => router.push('/dashboard/inventory/product-variants/new')

  const openEdit = (v: ProductVariant) => router.push(`/dashboard/inventory/product-variants/${v.id}/edit`)
