...
export default function AdminCustomersPage() {
  ...
  const { data: customers, loading, refresh } = useResource<User[]>('/api/dashboard/users')
  const { confirmDelete, deleteModal } = useConfirmDelete({ apiEndpoint: '/api/dashboard/users', entityName: 'customer', refresh, useQueryParam: true })

  const openCreate = () => router.push('/dashboard/customers/new')

  const openEdit = (u: User) => router.push(`/dashboard/customers/${u.id}/edit`)
