...
export default function AdminInvoicesPage() {
  ...
  const { data: invoices, loading, refresh } = useResource<Invoice[]>('/api/dashboard/invoices')
  const { confirmDelete, deleteModal } = useConfirmDelete({ apiEndpoint: '/api/dashboard/invoices', entityName: 'invoice', refresh, useQueryParam: true })

  const openCreate = () => router.push('/dashboard/sales/invoices/new')

  const openEdit = (inv: Invoice) => router.push(`/dashboard/sales/invoices/${inv.id}/edit`)
