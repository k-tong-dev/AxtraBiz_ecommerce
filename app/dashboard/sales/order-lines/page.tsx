...
export default function AdminOrderLinesPage() {
  ...
  const { data: items, loading, refresh } = useResource<OrderLine[]>('/api/dashboard/order-lines')
  const { confirmDelete, deleteModal } = useConfirmDelete({ apiEndpoint: '/api/dashboard/order-lines', entityName: 'order line', refresh, useQueryParam: true })

  const openCreate = () => router.push('/dashboard/sales/order-lines/new')

  const openEdit = (item: OrderLine) => router.push(`/dashboard/sales/order-lines/${item.id}/edit`)
