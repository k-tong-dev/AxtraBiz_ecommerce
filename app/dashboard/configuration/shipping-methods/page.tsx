...
export default function AdminShippingMethodsPage() {
  ...
  const { data: shippingMethods, loading, refresh } = useResource<ShippingMethod[]>('/api/dashboard/shipping-methods')
  const { confirmDelete, deleteModal } = useConfirmDelete({ apiEndpoint: '/api/dashboard/shipping-methods', entityName: 'shipping method', refresh, useQueryParam: true })

  const openCreate = () => router.push('/dashboard/configuration/shipping-methods/new')
  const openEdit = (row: ShippingMethod) => router.push(`/dashboard/configuration/shipping-methods/${row.id}/edit`)
