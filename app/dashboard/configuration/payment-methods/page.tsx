...
export default function AdminPaymentMethodsPage() {
  ...
  const { data: paymentMethods, loading, refresh } = useResource<PaymentMethod[]>('/api/dashboard/payment-methods')
  const { confirmDelete, deleteModal } = useConfirmDelete({ apiEndpoint: '/api/dashboard/payment-methods', entityName: 'payment method', refresh, useQueryParam: true })

  const openCreate = () => router.push('/dashboard/configuration/payment-methods/new')
  const openEdit = (row: PaymentMethod) => router.push(`/dashboard/configuration/payment-methods/${row.id}/edit`)
