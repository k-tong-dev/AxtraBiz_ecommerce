...
export default function AdminCurrenciesPage() {
  ...
  const { data: currencies, loading, refresh } = useResource<any[]>('/api/dashboard/currencies')
  const { confirmDelete, deleteModal } = useConfirmDelete({ apiEndpoint: '/api/dashboard/currencies', entityName: 'currency', refresh, useQueryParam: false })

  const openCreate = () => router.push('/dashboard/configuration/currencies/new')
  const openEdit = (row: any) => router.push(`/dashboard/configuration/currencies/${row.code}/edit`)
