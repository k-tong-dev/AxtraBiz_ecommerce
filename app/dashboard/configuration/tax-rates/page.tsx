...
export default function AdminTaxRatesPage() {
  ...
  const { data: taxRates, loading, refresh } = useResource<TaxRate[]>('/api/dashboard/tax-rates')
  const { confirmDelete, deleteModal } = useConfirmDelete({ apiEndpoint: '/api/dashboard/tax-rates', entityName: 'tax rate', refresh, useQueryParam: true })

  const openCreate = () => router.push('/dashboard/configuration/tax-rates/new')
  const openEdit = (row: TaxRate) => router.push(`/dashboard/configuration/tax-rates/${row.id}/edit`)
