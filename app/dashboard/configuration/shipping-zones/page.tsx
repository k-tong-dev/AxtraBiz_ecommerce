...
export default function AdminShippingZonesPage() {
  ...
  const { data: zones, loading, refresh } = useResource<ShippingZone[]>('/api/dashboard/shipping-zones')
  const { confirmDelete, deleteModal } = useConfirmDelete({ apiEndpoint: '/api/dashboard/shipping-zones', entityName: 'shipping zone', refresh, useQueryParam: true })

  const openCreate = () => router.push('/dashboard/configuration/shipping-zones/new')
  const openEdit = (row: ShippingZone) => router.push(`/dashboard/configuration/shipping-zones/${row.id}/edit`)
