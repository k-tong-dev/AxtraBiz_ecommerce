...
export async function GET() {
  try {
    const all = await fetchProductReviewsFromDrizzle()
...
export async function POST(request: Request) {
  try {
    ...
    for (const item of items) {
      const r = await productReviewService.upsert(item)
      if (!r.success) {
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!id) {
      return NextResponse.json({ error: 'Product review ID is required' }, { status: 400 })
    }
