...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const item = await fetchProductVariantFromDrizzle(id)
    if (!item) return NextResponse.json({ error: 'Variant not found' }, { status: 404 })
    return NextResponse.json(item)
  } catch { return NextResponse.json({ error: 'Failed to fetch variant' }, { status: 500 }) }
}
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await productVariantService.write(id, { ...body, id }, userId)
    if (result.success) {
      const updated = await fetchProductVariantFromDrizzle(id)
