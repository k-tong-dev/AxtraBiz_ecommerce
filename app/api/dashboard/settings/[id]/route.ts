...
import { fetchIrUserConfigFromDrizzle, irUserConfigService, deleteIrUserConfigFromDrizzle } from '@/lib/drizzle/queries/ir-user-config'
import { getCurrentUserId } from '@/utils/supabase/current-user'

export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    const { id } = await params
    const item = await fetchIrUserConfigFromDrizzle(id)
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await irUserConfigService.write(id, { ...body, id }, userId)
    if (result.success) {
      const updated = await fetchIrUserConfigFromDrizzle(id)
